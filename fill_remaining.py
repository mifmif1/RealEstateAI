...
def extract_floor(text):
    ...
    floor_patterns = [
        ...
        (r'\b5th\s+floor\b', '5th floor'),
        (r'\b6th\s+floor\b', '6th floor'),
...
if os.path.exists(excel_path):
    ...
    # Count empty cells
    empty_sqm = df['sqm'].isna().sum()
    empty_floor = df['floor'].isna().sum()
    
    print('Empty cells before extraction:')
    print('  Sqm: ' + str(empty_sqm))
    print('  Floor: ' + str(empty_floor))
    print()
    ...
    for idx, row in df.iterrows():
        ...
        # Extract sqm only if currently empty
        if pd.isna(row['sqm']):
            sqm_value = extract_sqm(description)
            if sqm_value is not None:
                df.at[idx, 'sqm'] = sqm_value
                sqm_count += 1
                if len(sqm_samples) < 8:
                    desc_preview = str(description)[:75] + '...' if len(str(description)) > 75 else description
                    sqm_samples.append((asset_id, desc_preview, sqm_value))
        
        # Extract floor only if currently empty
    ...
    print('  Total sqm filled: ' + str(df_check['sqm'].notna().sum()))
    print('  Total floor filled: ' + str(df_check['floor'].notna().sum()))
    print()
