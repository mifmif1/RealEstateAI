...
class SpitogatosData:
    ...
    def get_by_location(self, location: Rectangle, min_area: int = None,
        ...
        self.params['livingAreaLow'] = str(min_area)  if min_area else None
        self.params['livingAreaHigh'] = str(max_area) if max_area else None
        self.params['latitudeLow'] = str(location.min_lat)[:9] if location.min_lat else None
        self.params['latitudeHigh'] = str(location.max_lat)[:9] if location.max_lat else None
        self.params['longitudeLow'] = str(location.min_lon)[:9] if location.min_lon else None
        self.params['longitudeHigh'] = str(location.max_lon)[:9] if location.max_lon else None

        self.headers["Referer"] = "https://www.spitogatos.gr/en/for_sale-homes/map-search" + (f"/minliving_area-{min_area}" if min_area else '')+ (f"/maxliving_area-{max_area}" if max_area else '')+"?"+f"latitudeLow={self.params['latitudeLow']}&latitudeHigh={self.params['latitudeHigh']}&longitudeLow={self.params['latitudeLow']}&longitudeHigh={self.params['latitudeHigh']}&zoom={self.params['zoom']}"
